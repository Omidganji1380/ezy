<div>
    <div class="container">
        <div class="row justify-content-center">
            @foreach($profiles as $item)
                <div class="col-md-6 col-lg-4 col-xxl-3 my-4">
                    <div class="card" style="border-radius: 20px;box-shadow: rgba(0,0,0,0.2) 0 0 20px">
                        <div class="container" style="">
                            {{--<div class="card-header">
                                <div class="row">
                                    <div class="col-6">
                                        <button class="btn w-100">
                                            <i class="icofont-link text-info"></i>
                                            زیلینک
                                        </button>
                                    </div>
                                    <div class="col-6">
                                        <button class="btn w-100">
                                            <i class="icofont-link text-info"></i>
                                            فروشگاه
                                        </button>
                                    </div>
                                </div>
                            </div>--}}
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="row p-2 justify-content-end"
                                             style="background: rgb(248,248,248);border-radius: 8px">
                                            <div class="col-auto px-1 align-self-center">
                                                ezy.company/{{$item->link}}</div>
                                            <div class="col-auto px-1 align-self-center">|</div>
                                            <div class="col-auto px-1">
                                                <button class="btn p-1 text-info">
                                                    <i class="icofont-share"></i>
                                                </button>
                                            </div>
                                            <div class="col-auto px-1">
                                                <button class="btn p-1 text-info">
                                                    <i class="icofont-ui-copy"></i>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 my-2">
                                        <div class="row border p-2 justify-content-end"
                                             style="border-radius: 8px">
                                            <div class="col-4 align-self-center">
                                                <img
                                                    src="{{asset(!$item->img?'userGray.png':'storage/pb/profiles/profile-'.$item->id.'/'.$item->img)}}"
                                                    class="w-100 p-3">
                                            </div>
                                            <div class="col-8 align-self-center">
                                                <div class="row">
                                                    <div class="col-12">
                                                        <h5 class="font-weight-bold">{{$item->title}}</h5>
                                                    </div>
                                                    <div class="col-12">
                                                        <p class="m-0">{{$item->subtitle}}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <div class="row p-2 justify-content-between"
                                             style="background: rgb(248,248,248);border-radius: 8px">
                                            <div class="col-auto px-1 align-self-center"><i
                                                    class="icofont-ui-clip-board me-2"></i>نرم‌افزار و خدمات اینترنتی
                                            </div>
                                            <div class="col-auto px-1 align-self-center">
                                                <button class="btn p-1 text-info">
                                                    <i class="icofont-ui-edit"></i>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer mx-4 border-top-0"
                         style="border-radius: 0 0 20px 20px;box-shadow: rgba(0,0,0,0.2) 0 0 20px">
                        <div class="row justify-content-evenly">
                            <button class="col-3 btn-light p-0 py-2 btn">
                                <div class="row">
                                    <div class="col-12">
                                        <i class="icofont-chart-histogram text-info"></i>
                                    </div>
                                    <div class="col-12 text-info">
                                        گزارش آماری
                                    </div>
                                </div>
                            </button>
                            <a href="{{route('pb.show',$item->link)}}" target="_blank" class="col-3 w-auto">
                                <button class="w-full h-full btn-light p-0 py-2 btn">
                                    <div class="row">
                                        <div class="col-12">
                                            <i class="icofont-eye text-info"></i>
                                        </div>
                                        <div class="col-12 text-info">
                                            نمایش
                                        </div>
                                    </div>
                                </button>
                            </a>
                            <button class="col-3 btn-light p-0 py-2 btn" wire:click="edit({{$item->id}})">
                                <div class="row">
                                    <div class="col-12">
                                        <i class="icofont-pencil text-info"></i>
                                    </div>
                                    <div class="col-12 text-info">
                                        ویرایش
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
            @endforeach
        </div>
    </div>
    <div class="fixed-bottom" style="width: fit-content">
        <button class="btn btn-info fs-2 p-4 rounded-circle"
                data-bs-toggle="modal"
                data-bs-target="#insertProfile"
                style="margin: 0 60px 60px 0"><i class="icofont-plus fs-3 text-white"></i></button>
    </div>
    @push('css')
        <link rel="stylesheet" href="{{asset('pageBuilder/assets/css/tailwind.min.css')}}">
        {{--        <script src="{{asset('pageBuilder/assets/js/alpine.js')}}" defer></script>--}}
        <style>
            [x-cloak] {
                display: none;
            }

            [type="checkbox"] {
                box-sizing: border-box;
                padding: 0;
            }

            .form-checkbox,
            .form-radio {
                -webkit-appearance: none;
                -moz-appearance: none;
                appearance: none;
                -webkit-print-color-adjust: exact;
                color-adjust: exact;
                display: inline-block;
                vertical-align: middle;
                background-origin: border-box;
                -webkit-user-select: none;
                -moz-user-select: none;
                -ms-user-select: none;
                user-select: none;
                flex-shrink: 0;
                color: currentColor;
                background-color: #fff;
                border-color: #e2e8f0;
                border-width: 1px;
                height: 1.4em;
                width: 1.4em;
            }

            .form-checkbox {
                border-radius: 0.25rem;
            }

            .form-radio {
                border-radius: 50%;
            }

            .form-checkbox:checked {
                background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M5.707 7.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L7 8.586 5.707 7.293z'/%3e%3c/svg%3e");
                border-color: transparent;
                background-color: currentColor;
                background-size: 100% 100%;
                background-position: center;
                background-repeat: no-repeat;
            }

            .form-radio:checked {
                background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
                border-color: transparent;
                background-color: currentColor;
                background-size: 100% 100%;
                background-position: center;
                background-repeat: no-repeat;
            }
        </style>
    @endpush
    <div class="modal fade rounded" wire:ignore.self id="insertProfile" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="border-color: #c5c5c5 !important;">
                    <h5 class="modal-title mx-auto">ساخت پروفایل جدید</h5>
                    <button type="button" style="width: 20px;height: 20px;"
                            class="me-1 close btn border-dark border-2 rounded-circle p-0"
                            data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true" class="fa fa-close">{{--&times;--}}</span>
                    </button>
                </div>
                <div class="modal-body" style="background-color: rgb(241, 243, 246);">
                    <div class="antialiased sans-serif bg-gray-200">
                        <div x-data="app()" x-cloak>
                            <div class="max-w-3xl mx-auto px-4">

                                <div x-show.transition="step === 'complete'">
                                    <div
                                        class="bg-white rounded-lg p-10 flex items-center shadow justify-between justify-content-center">
                                        <div>
                                            <h2 class="text-2xl mb-4 text-gray-800 text-center font-bold">چند لحظه صبر
                                                کنید</h2>
                                        </div>
                                    </div>
                                </div>

                                <div x-show.transition="step != 'complete'">
                                    <!-- Top Navigation -->
                                    <div class="border-b-2 py-3">
                                        <div
                                            class="uppercase tracking-wide text-xs font-bold text-gray-500 mb-1 leading-tight"
                                            x-text="`قدم: ${step}`"></div>
                                        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                            <div class="flex-1">
                                                <div x-show="step === 1">
                                                    <div class="text-lg font-bold text-gray-700 leading-tight">آدرس
                                                        پروفایل
                                                    </div>
                                                </div>

                                                <div x-show="step === 2">
                                                    <div class="text-lg font-bold text-gray-700 leading-tight">مشخصات
                                                    </div>
                                                </div>

                                                <div x-show="step === 3">
                                                    <div class="text-lg font-bold text-gray-700 leading-tight">حوزه
                                                        فعالیت
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="flex items-center md:w-64">
                                                <div class="w-full bg-white rounded-full mr-2">
                                                    <div
                                                        class="rounded-full bg-green-500 text-xs leading-none h-2 text-center text-white"
                                                        :style="'width: '+ parseInt(step / 3 * 100) +'%'"></div>
                                                </div>
                                                <div class="text-xs w-10 text-gray-600"
                                                     x-text="parseInt(step / 3 * 100) +'%'"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- /Top Navigation -->

                                    <!-- Step Content -->
                                    <div class="py-10">
                                        <div x-show.transition.in="step === 1">
                                            <div>
                                                <label for="firstname" class="font-bold mb-1 text-gray-700 block">آدرس
                                                    پروفایل</label>
                                                <div
                                                    class="row w-full justify-content-end mx-auto py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium bg-white">
                                                    <div class="col p-0">
                                                        <input type="text" dir="ltr"
                                                               class="focus:outline-none text-gray-600 font-medium w-full"
                                                               wire:model.live.debounce="url">
                                                    </div>
                                                    <div class="col-auto" dir="ltr">
                                                        ezy.company/
                                                    </div>
                                                </div>
                                                @error('url')
                                                <div
                                                    class="row w-full justify-content-center mx-auto py-1 rounded-lg shadow-sm focus:outline-none focus:shadow-outline bg-white my-2">
                                                    <div class="col">
                                                        <p class="text-danger text-center">{{$message}}</p>
                                                    </div>
                                                </div>
                                                @enderror
                                            </div>
                                        </div>
                                        <div x-show.transition.in="step === 2">

                                            <div class="mb-5 text-center">
                                                <label class="font-bold mb-1 text-gray-700 block text-right">بارگذاری
                                                    عکس پروفایل</label>
                                                <div wire:ignore
                                                    class="mx-auto w-full h-full mb-2 border rounded-3 relative bg-green-100 {{--bg-gray-100--}} mb-4 shadow-inset {{--bg-transparent--}}">
                                                    <img id="image"
                                                         class="h-32 rounded-3 absolute left-0 right-0 mx-auto"
                                                         :src="image" alt=" "/>
                                                    <label for="fileInput" type="button"
                                                           class="opacity-0 w-full h-32"></label>
                                                </div>

                                                <input name="photo" id="fileInput" accept="image/*" class="hidden"
                                                       type="file" wire:model.blur="img"
                                                       @change="let file = document.getElementById('fileInput').files[0];
								var reader = new FileReader();
								reader.onload = (e) => image = e.target.result;
								reader.readAsDataURL(file);">

                                                <div class="relative text-right my-4">
                                                    <label class="font-bold mb-1 text-gray-700 block">عنوان پروفایل
                                                        <span class="text-danger">*</span></label>
                                                    <input type="text" wire:model.live="title"
                                                           class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium"
                                                           placeholder="عنوان پروفایل">
                                                </div>
                                                <div class="relative text-right my-4">
                                                    <label class="font-bold mb-1 text-gray-700 block">زیرعنوان
                                                        پروفایل</label>
                                                    <input type="text" wire:model="subtitle"
                                                           class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium"
                                                           placeholder="زیرعنوان پروفایل">
                                                </div>
                                            </div>

                                        </div>
                                        <div x-show.transition.in="step === 3">
                                            <div class="mb-5">
                                                <div class="row">
                                                    <div class="col-12 relative">
                                                        <input type="text" class="form-control pr-10 py-3"
                                                               placeholder="جستجو ...">
                                                        <i class="icofont-search mr-3 my-auto bottom-0 top-0 absolute"
                                                           style="height: min-content"></i>
                                                    </div>
                                                    <div class="col-12 my-4 rounded">
                                                        <div class="bg-gray-300 text-center py-3 rounded">
                                                            <p class="fs-4 mb-3">فروشگاه و فروش کالا</p>
                                                            <p>فعالیت‌هایی که به فروش کالای فیزیکی منجر می‌شوند.</p>
                                                        </div>
                                                        <ul class="list-group rounded my-4">
                                                            <li class="list-group-item rounded my-1 relative"
                                                                style="border: 1px solid #c5c5c5">
                                                                <i class="icofont-baby-cloth my-auto bottom-0 top-0 absolute"
                                                                   style="height: min-content"></i>
                                                                <p class="fs-6 font-bold pr-8">پوشاک و اکسسوری</p>
                                                                <p class=" pr-8">(فروشگاه لباس، کیف و کفش، اکسسوری، ساعت
                                                                    و ...)</p>
                                                            </li>
                                                            <li class="list-group-item rounded my-1 relative"
                                                                style="border: 1px solid #c5c5c5">
                                                                <i class="icofont-mathematical my-auto bottom-0 top-0 absolute"
                                                                   style="height: min-content"></i>
                                                                <p class="fs-6 font-bold pr-8">لوازم آرایشی و
                                                                    بهداشتی</p>
                                                                <p class=" pr-8">(فروش محصولات مراقبت پوست و زیبایی و
                                                                    ...)</p>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <!-- / Step Content -->
                                    </div>
                                </div>

                                <!-- Bottom Navigation -->
                                <div class="bottom-0 left-0 right-0 py-3 rounded-2xl bg-white shadow-md"
                                     x-show="step != 'complete'">
                                    <div class="max-w-3xl mx-auto px-4">
                                        <div class="row justify-around flex-wrap-reverse">
                                            <div class="col-sm-6 my-2">
                                                <button x-show="step > 1" @click="step--"
                                                        class="w-full focus:outline-none py-2 px-2 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border">
                                                    قدم قبل
                                                </button>
                                            </div>
                                            <div class="col-sm-6 my-2 text-end">

                                                <button x-show="step === 1" @click="step++"
                                                        {{!$nextButton?'disabled':''}}
                                                        class="w-full focus:outline-none border border-transparent py-2 {{!$nextButton?'opacity-25':''}} px-2 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">
                                                    قدم بعد
                                                </button>

                                                <button x-show="step === 2" @click="step++"
                                                        {{!$nextButton2?'disabled':''}}
                                                        class="w-full focus:outline-none border border-transparent py-2 {{!$nextButton2?'opacity-25':''}} px-2 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">
                                                    قدم بعد
                                                </button>

                                                <button @click="step = 'complete'" x-show="step === 3"
                                                        wire:click="submit"
                                                        class="w-full focus:outline-none border border-transparent py-2 px-2 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">
                                                    تمام
                                                </button>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                                <!-- / Bottom Navigation https://placehold.co/300x300/e2e8f0/cccccc -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    @push('js')
        <script>
            $(window).ready(function () {
                $('i').addClass('fs-5')
                // $('button').addClass('fs-5')
                // if ($('#image').attr('src', 'image')) {
                //     $('#image').addClass('opacity-0')
                // } else {
                //     $('#image').removeClass('opacity-0')
                // }
            })

            function app() {
                return {
                    step                : 1,
                    // passwordStrengthText: '',
                    // togglePassword      : false,
                    image               : 'data:image/jpeg;base64,',
                    // // image               : 'image',
                    // password: '',
                    // gender  : 'Male',
                    checkPasswordStrength() {
                        var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
                        var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");
                        let value       = this.password;
                        if (strongRegex.test(value)) {
                            this.passwordStrengthText = "Strong password";
                        } else if (mediumRegex.test(value)) {
                            this.passwordStrengthText = "Could be stronger";
                        } else {
                            this.passwordStrengthText = "Too weak";
                        }
                    }
                }
            }
        </script>
    @endpush
</div>
